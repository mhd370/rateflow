...
export default function RegisterPage() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      setError(String(err?.message || t("auth.registrationFailed", "Registration failed.")));
    } finally {
      setLoading(false);
    }
  }
            ...
            <Stack spacing={2.2}>
              <Box>
                <Stack direction="row" spacing={1} alignItems="center">
                  <PersonAddAltOutlinedIcon sx={{ color: "#39c6ff" }} />
                  <Typography sx={{ fontWeight: 1000, fontSize: { xs: 22, md: 26 } }}>
                    {t("auth.createAccountHeading", "Create account")}
                  </Typography>
                </Stack>
                <Typography sx={{ mt: 0.8, opacity: 0.72, fontSize: 13, lineHeight: 1.6 }}>
                  {t(
                    "auth.registerSubtitle",
                    "Register with your name, email, and a password (min 8 characters).",
                  )}
                </Typography>
              </Box>
